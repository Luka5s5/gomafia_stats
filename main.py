...
from db_manager import (init_database, insert_full_tournament,
                        insert_player_data, user_exists)
    ...
    async def process_one(tournament_id: int):
        async with semaphore:
            try:
                ...
                if tournament_data:
                    # insert_player_data is synchronous – run in a thread
                    await asyncio.to_thread(insert_full_tournament, *tournament_data)
...
async def main():
    ...
    async with HttpClient() as client:
        ...
        #
        # print(await scrape_tournament(client, 2628))
        await scrape_tournaments_range(client, 1, 2629)
